...
MoonAnime — після підтвердження у браузері, або теж у **mpv**,
якщо увімкнути експериментальне налаштування (див. нижче).
...
| | |
| :--- | :--- |
| 🔍 **Пошук** | Строгий (≤20) або розширений (≤100) · групування франшиз · відлік до наступної серії |
| 📖 **Бібліотека** | Статуси, фільтри, resume, позначка «переглянуто» · експорт у JSON |
| ▶️ **Відтворення** | Ashdi → mpv · MoonAnime → браузер або mpv (експеримент) · autoplay |
| 🎚 **Якість** | Авто / максимальна / мінімальна — через `--hls-bitrate` для mpv |
| 📄 **Опис** | `i` показує синопсис обраного тайтла |
| 📊 **Статистика** | Переглянуті серії, тайтли за статусами, приблизний час перегляду |
...
| ⌨️ **Клавіші** | Шорткати на **англійській** та **українській** розкладках |

### MoonAnime у mpv (експеримент)

**Налаштування → Основні → ІНТЕГРАЦІЇ → MoonAnime у mpv** — типово вимкнено.

MoonAnime не віддає посилання на потік так, як Ashdi: сторінка плеєра ховає
його за двома шарами обфускації. anihub-cli розшифровує їх без жодних
додаткових програм і далі грає серію в mpv зі звичайним прогресом, resume,
автопереходом і — де MoonAnime їх дає — субтитрами (підключені, але не
увімкнені; `j` у mpv перемикає).

Обфускація зроблена навмисно, тож очікуй, що це зламається, коли MoonAnime
перебудує сторінку. Нічого при цьому не втрачається: натисни `o`, щоб відкрити
серію в браузері, як і раніше.
